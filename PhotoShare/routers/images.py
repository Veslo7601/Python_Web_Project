...
@router.post(
    "/", response_model=ImageResponseSchema, status_code=status.HTTP_201_CREATED, dependencies=[Depends(RateLimiter(times=2, seconds=15))]
)
...
@router.get(
    "/images", response_model=List[ImageResponseSchema], status_code=status.HTTP_200_OK, dependencies=[Depends(RateLimiter(times=2, seconds=15))]
)
...
@router.get(
    "/", response_model=ImageResponseSchema, status_code=status.HTTP_200_OK, dependencies=[Depends(RateLimiter(times=2, seconds=15))]
)
async def get_image(
        image_path: str,
        db: AsyncSession = Depends(get_database), user: User = Depends(auth_service.get_current_user)
